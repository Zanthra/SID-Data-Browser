/*
*  This class has been modified from the original by Scott Winegarden.  The purpose of this change
*  is to make use of the more generic java Calendar objects, and to integrate it further into the
*  Sid Data Browser.
*
*  by: Scott Winegarden scottw@sun.stanford.edu
*/
...
public class SunriseSunset
{
	...
	private Calendar	dateInput;				// date/time from user
	private double	dfTimeZone;				// time zone from user

	private Calendar	dateSunrise;			// date and time of sunrise
	private Calendar	dateSunset;				// date and time of sunset
...
*----------------------------------------------------------------------------*/
	...
	{
		iYear = dateInput.get(Calendar.YEAR);
		iMonth = dateInput.get(Calendar.MONTH) + 1;
		iDay = dateInput.get(Calendar.DATE);
		...
		if( bSunriseToday )
		{
			dateSunrise = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
			dateSunrise.clear();
			dateSunrise.set(iYear, iMonth - 1, iDay, (int)dfHourRise, (int)dfMinRise);
		}
		...
		if( bSunsetToday )
		{
			dateSunset = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
			dateSunset.clear();
			dateSunset.set(iYear, iMonth - 1, iDay, (int)dfHourSet, (int)dfMinSet);
		}
	}
